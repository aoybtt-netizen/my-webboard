...
const UPGRADE_CONFIG = {
    'ship engine': {
        // stat ที่เพิ่มต่อ 1 level (ค่าติดลบ = ดีขึ้น เช่น consumption ลด)
        statPerLevel: {
            power:       +20,
            consumption: -5
        },
        // ค่าใช้จ่ายตาม level ปัจจุบัน (lv = level ก่อนอัปเกรด)
        costPerLevel: (lv) => ({
            metal:  lv * 3,
            energy: lv * 2,
            tech:   0
    ...
    'card barrier': {
        statPerLevel: {
            maxShield: +5,
            shield:    +5,  // เพิ่มค่าปัจจุบันด้วยเพื่อให้ใช้งานได้ทันที
            recharge:  +0.3
        },
        costPerLevel: (lv) => ({
            metal:  0,
    ...
    'turret': {
        statPerLevel: {
            damage:   +5,
            fireRate: -0.05  // ยิ่งต่ำยิ่งยิงเร็ว
        },
        costPerLevel: (lv) => ({
            metal:  lv * 2,
            energy: lv * 2,
...
app.post('/api/:mode/game/upgrade-item', async (req, res) => {
    ...
    try {
        ...
        // --- 2. ตรวจสอบอุปกรณ์ในช่อง ---
        const item = user.equipped?.[slot];
        if (!item) {
            return res.json({ success: false, message: 'ไม่มีอุปกรณ์ในช่องนี้' });
        }

        // --- 3. เช็ก Max Level ---
        if (item.level >= item.maxUpgrades) {
            return res.json({ success: false, message: 'อุปกรณ์ถึง MAX LEVEL แล้ว!' });
        }

        // --- 4. ดึง Config ของประเภทนี้ ---
        const config = UPGRADE_CONFIG[item.type];
        if (!config) {
            return res.json({ success: false, message: `ไม่รองรับการอัปเกรดประเภท: ${item.type}` });
        }

        // --- 5. คำนวณต้นทุน (ใช้ level ปัจจุบัน) ---
        const cost = config.costPerLevel(item.level);

        // --- 6. เช็กทรัพยากรผู้เล่น ---
        const userMetal  = user.metal  ?? 0;
        const userEnergy = user.energy ?? 0;
        const userTech   = user.tech   ?? 0;

        const hasEnough =
            userMetal  >= cost.metal &&
            userEnergy >= cost.energy &&
            userTech   >= cost.tech;

        if (!hasEnough) {
            return res.json({
                success: false,
                message: 'ทรัพยากรไม่เพียงพอ!',
                required: cost,
                current: { metal: userMetal, energy: userEnergy, tech: userTech }
            });
        }

        // --- 7. คำนวณ Stats ใหม่หลังอัปเกรด ---
        const newItem = { ...item, level: item.level + 1 };

        for (const [stat, delta] of Object.entries(config.statPerLevel)) {
            if (newItem[stat] !== undefined) {
                // ปัดทศนิยม 2 ตำแหน่งเพื่อป้องกัน floating point
                newItem[stat] = Math.round((newItem[stat] + delta) * 100) / 100;
                // ป้องกันค่าติดลบ (เช่น fireRate หรือ consumption)
                if (newItem[stat] < 0) newItem[stat] = 0;
            }
        }

        // --- 8. อัปเดต DB: หักทรัพยากร + บันทึก item ใหม่ ---
        const incObj = {};
        if (cost.metal  > 0) incObj.metal  = -cost.metal;
        if (cost.energy > 0) incObj.energy = -cost.energy;
        if (cost.tech   > 0) incObj.tech   = -cost.tech;

        await db.updateOne(
            { username },
            {
                $inc: incObj,
                $set: { [`equipped.${slot}`]: newItem }
            }
        );

        // --- 9. ส่งผลลัพธ์กลับ ---
        console.log(`✅ UPGRADE: ${username} upgraded [${slot}] ${item.name} Lv${item.level} → Lv${newItem.level}`);

        return res.json({
            success: true,
            message: `อัปเกรด ${item.name} เป็น Level ${newItem.level} สำเร็จ!`,
            newItem,
            costPaid: cost,
            newResources: {
                metal:  userMetal  - (cost.metal  || 0),
                energy: userEnergy - (cost.energy || 0),
                tech:   userTech   - (cost.tech   || 0)
            }
        });
