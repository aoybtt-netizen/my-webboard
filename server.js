...
app.get('/api/merchant/tasks', async (req, res) => {
    ...
    try {
        // 1. ดึงงานที่ยังไม่ใช่ 'closed_by_merchant'
        ...
        const activeTasks = posts.filter(post => {
            ...
            // กรองงานที่หมดเวลาและไม่มีคนรับออก
            if (isExpiredAndNoRider) return false;

            // ✅ เงื่อนไขสำคัญ: ถ้าสถานะคือ 'closed_by_merchant' ให้หายไปเลย (ตาม Query ด้านบน)
            // แต่ถ้าสถานะเป็นอย่างอื่น (เช่น finished, closed_permanently จาก bypass) 
            // ให้เช็คต่อว่า "ให้คะแนนหรือยัง?"
            
            const hasNotRated = !post.merchantRating; // เช็คว่ายังไม่ได้ให้คะแนน (เป็น null หรือ undefined)

            // ถ้ายังไม่ให้คะแนน ให้โชว์งานไว้เสมอ เพื่อให้ร้านเห็นปุ่ม "จบงาน & ให้คะแนน"
            if (hasNotRated) {
                return true;
            }

            // ถ้าให้คะแนนแล้วแต่งานยังค้างสถานะอื่นอยู่ ก็ให้หายไปได้เลย
            return false;
        });
